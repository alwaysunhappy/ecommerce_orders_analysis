...
from src.build_mart import build_orders_mart
...
def run_pipeline(raw_dir: Path = RAW_DIR, db_path: Path = DB_PATH) -> None:
    ...
    print("Step 1/5: loading CSV files to SQLite")
    load_csvs_to_sqlite(raw_dir, db_path)

    print("Step 2/5: building orders_mart")
    build_orders_mart(db_path)

    print("Step 3/5: calculating metrics")
    save_metrics(db_path)

    print("Step 4/5: creating figures")
    make_figures(db_path)

    print("Step 5/5: running hypothesis tests")
